...
struct ThunderbirdDeviceImageTy : public DeviceImageTy {
  ...
  ThunderbirdDeviceImageTy(int32_t ImageId, GenericDeviceTy &Device,
      ...
      : DeviceImageTy(ImageId, Device, std::move(Image)), DynLib() {}
...
struct ThunderbirdDeviceTy : public GenericDeviceTy {
  ...
  loadBinaryImpl(std::unique_ptr<MemoryBuffer> &&TgtImage,
    ...
    DP("Image info: ImageStart=%p, Size=%zu bytes\n",
       (const void *)TgtImage->getBufferStart(), ImageSize);
    ...
    if (status != TBIRD_SUCCESS) {
      ...
      return Discard(Plugin::error(ErrorCode::UNKNOWN,
  ...
  /// host and written through the buffer it belongs to: a device pointer is a
  /// host address inside a pool slab, and writing through it directly does not
  /// reach the device's view of that slab.
  Error dataFillImpl(void *TgtPtr, const void *PatternPtr, int64_t PatternSize,
    ...
    if (!buffer)
      return Plugin::error(ErrorCode::INVALID_ARGUMENT,
                           "dataFill: pointer %p not in buffer registry",
                           TgtPtr);
...
public:
  Error getGlobalMetadataFromDevice(GenericDeviceTy &GenericDevice,
    ...
    // Resolve the symbol in the image's own ELF symbol table; its value is a
    // virtual address, relocated to where the image sits in the pool.
    ...
    uint64_t symbol_offset = *ValueOrErr - ThunderbirdImage.MinVMA;
    DeviceGlobal.setPtr((void *)(DeviceImageBase + symbol_offset));
    return Plugin::success();
  }
};
