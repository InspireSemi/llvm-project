...
KernelSet llvm::omp::getDeviceKernels(Module &M) {
  ...
  // Thunderbird kernels use the C calling convention, so the calling-convention
  // test does not see them; they were internalized and then dropped by GlobalDCE
  // at -O1+. Admit them by attribute instead, restricted to Inspire device
  // modules so every other target keeps upstream's selection -- including the
  // statistic for kernels that are not OpenMP target regions.
  const bool AdmitByAttribute =
      isOpenMPDevice(M) && M.getTargetTriple().getVendor() == Triple::Inspire;

  for (Function &F : M) {
    if (!F.hasKernelCallingConv() && !(AdmitByAttribute && isOpenMPKernel(F)))
      continue;
